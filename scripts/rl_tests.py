    ...
    assert isinstance(inputs_dict['gap_days_max'], (int)) and \
        int(inputs_dict['gap_days_max']) >= 0 and \
            int(inputs_dict['gap_days_min']) <= int(inputs_dict['gap_days_max']), \
                'must be greater than or equal to zero and gap_days_max'
...
def env_tests(gym_envs: Dict[str, list], inputs_dict: dict) -> NoReturn:
    """
    Conduct tests on all selected environments.
...
def method_checks(inputs_dict: dict) -> NoReturn:
    """
    Confirm the presence of numerous class methods required for learning.
