...
"""
title:                  lev_coin.py
usage:                  python scripts/lev_coin.py
python version:         3.9
torch verison:          1.9
...
if VRAM == 1:
    ...
    # still need 40GB of free RAM or reduce number of INVESTORS
...
ru_l, ru_h, ru_i = 0.20, 0.80, 0.005
rd_l, rd_h, rd_i = 0.20, 0.80, 0.005
...
def fixed_final_lev(outcomes: T.FloatTensor, top: int, value_0: T.FloatTensor, up_r: float, 
                    down_r: float, lev_low: float, lev_high: float, lev_incr: float):
    """
...
def smart_lev(outcomes: T.FloatTensor, investors: T.IntTensor, horizon: T.IntTensor, top: int, 
              value_0: T.FloatTensor, up_r: float, down_r: float, lev_low: float, 
...
def big_brain_lev(outcomes: T.FloatTensor, investors: T.IntTensor, horizon: T.IntTensor, 
                  top: int, value_0: T.FloatTensor, up_r: float, down_r: float, 
...
if __name__ == '__main__': 
    ...
    fixed_final_lev(outcomes, TOP, VALUE_0, UP_R, DOWN_R, lev_low=l0_l, lev_high=l0_h, lev_incr=l0_i)
    ...
    np.save(dir + 'coin_inv1_val.npy', inv1_val_data)
    np.save(dir + 'coin_inv1_val_T.npy', inv1_val_data_T)
    ...
    np.save(dir + 'coin_inv2_val.npy', inv2_val_data)
    ...
    np.save(dir + 'coin_inv3_val.npy', inv3_val_data)

    inv4_lev_data = galaxy_brain_lev(ru_min=ru_l, ru_max=ru_h, ru_incr=ru_i, rd_min=rd_l, rd_max=rd_h, 
                                     rd_incr=rd_i, pu_min=pu_l, pu_max=pu_h, pu_incr=pu_i)
    np.save(dir + 'coin_inv4_lev.npy', inv4_lev_data)
